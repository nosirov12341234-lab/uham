"""
Username Hunter Bot - To'liq versiya
SMS / Telegram / Email kod tanlash imkoniyati bilan
"""
...
def db_username_in_users(username):
    with sqlite3.connect(DB_FILE) as con:
        return con.execute(
            "SELECT 1 FROM userlar WHERE LOWER(username)=?", (username.lower(),)
        ).fetchone() is not None

def db_username_in_band(username):
    with sqlite3.connect(DB_FILE) as con:
        return con.execute(
    ...
    for _, telefon, tg_id, username, api_id, api_hash, _ in rows:
        ...
        try:
            ...
            if await client.is_user_authorized():
                ...
                loaded += 1
                log.info("Ulandi: %s @%s", telefon, username)
            else:
...
async def open_channel(username: str) -> bool:
    ...
    await bot.send_message(
        ...
        f"'🔄 Limitni tiklash' yoki yangi akkaunt qo'shing.",
        buttons=MAIN_KB,
    )
    return False
            ...
            for tg_id, username, ism, status in rows:
                ...
                try:
                    ...
                    if current and current != saved:
                        ...
                        await bot.send_message(
                            ADMIN_ID,
                            f"🔄 **@{username}** username o'zgartirdi!\n"
                            f"🔹 Eski: @{username}\n"
...
async def cmd_start(event):
    ...
    await event.reply(
        ...
        f"🔑 Akkauntlar: **{len(db_all_accounts())}** ta\n"
    ...
    async with bot.conversation(event.chat_id, timeout=300) as conv:
        try:
            ...
            try:
                ...
                await conv.send_message(f"❌ Ulanishda xato: {str(ex)[:150]}", buttons=ACCOUNTS_KB)
                return

            # 4. Kod yuborish usulini tanlash
            await bot.send_message(
                event.chat_id,
                "📨 **4-qadam: Kod qayerga yuborilsin?**",
                buttons=[
                    [Button.inline("✈️ Telegram ilovasiga", b"code_app"),
                     Button.inline("📱 SMS ga", b"code_sms")],
                ]
            )

            # Inline tugma bosilishini kutish
            code_type = None
            try:
                cb = await bot.wait_for(
                    events.CallbackQuery(from_users=ADMIN_ID),
                    timeout=60,
                )
                await cb.answer()
                code_type = cb.data
            ...
            # Kodni yuborish
            try:
                if code_type == b"code_sms":
                    sent = await client.send_code_request(telefon, force_sms=True)
                    await conv.send_message(
                        "📱 **SMS yuborildi!**\n\n"
                    ...
                    await conv.send_message(
                        "✈️ **Telegram ga yuborildi!**\n\n"
                        "Telegram ilovangizdagi kodni kiriting:\n"
                        "_(Masalan: 12345)_"
                    )
            except FloodWaitError as e:
                await conv.send_message(
                    f"⏳ Telegram server: {e.seconds} soniya kuting va qaytadan urining.",
                    buttons=ACCOUNTS_KB,
                )
                await client.disconnect()
                return
            except Exception as ex:
                await conv.send_message(f"❌ Kod yuborishda xato: {str(ex)[:200]}", buttons=ACCOUNTS_KB)
                await client.disconnect()
                return
            ...
            try:
                ...
                # 2FA
                await conv.send_message(
                    "🔐 **2FA (ikki bosqichli himoya) aniqlandi!**\n\n"
                ...
                # Email kod
                await conv.send_message(
                    "📧 **Email tasdiqlash kodi kerak!**\n\n"
                    "Emailingizga kelgan kodni kiriting:"
                )
                email_code_msg = await conv.get_response()
                try:
                    await client.sign_in(telefon, email_code=email_code_msg.text.strip())
                except Exception as ex:
                    await conv.send_message(
                        f"❌ Email kod xato: {str(ex)[:100]}",
                        buttons=ACCOUNTS_KB,
                    )
                    await client.disconnect()
                    return
                ...
                await conv.send_message(
                    "❌ Kod noto'g'ri!\n\nQaytadan '➕ Akkaunt ulash' tugmasini bosing.",
                    buttons=ACCOUNTS_KB,
                )
                await client.disconnect()
                return
...
async def main():
    ...
    try:
        await bot.send_message(
            ...
            f"⏱ Interval: **{db_get_interval()}** soniya\n\n"
            f"Akkaunt ulash:\n"
            f"🔑 Akkauntlar → ➕ Akkaunt ulash\n"
            f"API_ID/API_HASH: my.telegram.org dan oling",
            buttons=MAIN_KB,
        )
    except Exception as ex:
